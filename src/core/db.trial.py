...
def store_chunks(chunks: list[dict], doc_id: str) -> int:
    """Embed each chunk in batches and bulk-insert into the multimodal_chunks table."""
    if not chunks:
        return 0
    try:
# 1. Compute embeddings in safe batch sizes (e.g., 100 chunks per API call)
        BATCH_SIZE = 100
        all_embeddings = []
    ...
    # 2. Prepare tuples for bulk database insertion
        insert_data = []
        for chunk, emb in zip(chunks, all_embeddings):
            meta = chunk.get("metadata", {})

    # Ensure embedding is formatted as a 1D vector string
            # emb_str = str(emb[0] if isinstance(emb[0], list) else emb)

            emb_list = emb[0] if isinstance(emb[0], list) else emb
            emb_str = f"[{','.join(map(str,emb_list))}]"

    # Decode base64 image string to bytes if present
            # img_b64 = meta.get("image_base64")
            # img_bytes = base64.b64decode(img_b64) if img_b64 else None

            raw_page = meta.get("page_number")
            page_num = int(raw_page) if raw_page is not None and str(raw_page).isdigit else None

            position_json = json.dumps(meta.get("position")) if meta.get("position") else None
    ...
    # 3. Bulk insert rows in a single DB transaction
        with get_db_conn() as conn:
            # raw_conn = getattr(conn,"connection",conn)
            # if hasattr(raw_conn,"driver_connection"):
            #     raw_conn = raw_conn.driver_connection

            with conn.cursor() as cur:
                # Delete stale chunks for re-ingestion
                cur.execute("DELETE FROM multimodal_chunks WHERE doc_id = %s::uuid", (doc_id,))

                insert_query = """
                    INSERT INTO multimodal_chunks (
                    doc_id, content, embedding, chunk_type, element_type,
                    section, page_number, source_file, position, image_base64
                    ) VALUES (%s::uuid, %s, %s::vector, %s, %s, %s, %s::integer, %s, %s::jsonb, %s)
                    """

                # Fast bulk insert
                # execute_values(cur, insert_query, insert_data, template="(%s::uuid, %s, %s::vector, %s, %s, %s, %s::integer, %s, %s, %s)")
...
def get_db_connection_fts():
    return psycopg2.connect(pg_fts_connection)

def get_db_connection():
    return psycopg2.connect(pg_vector_connection)