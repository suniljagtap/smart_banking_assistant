...
from src.api.v1.tools.tools import vector_search_tool, fts_search_tool, hybrid_search_tool
import cohere
...
def reranker(state: RAGState):
   ...
   top_score = rerank_response.results[0].relevance_score if rerank_response.results else 0.0
...
def response_generator_node(state: RAGState) -> RAGState:
   ...
   answer = nl_chain.invoke(
       {"query": state["query"], "sql": query, "sql_result": sql_result, "search_result":search_result}
   )
...
def build_rag_graph():
   ...
   workflow.add_conditional_edges(
       ...
       {"VECTOR_DB": "rag_retriever", "RDBMS": "sql_generator","CHIT_CHAT":"response_generator"},
   )
   ...
   workflow.add_conditional_edges("sql_execution",
                                  route_after_execution,
                                  {
                                     ...
                                     "resposne_generator" : "response_generator"
                                  })

   workflow.add_conditional_edges("reranker",
                                     evaluate_quality,
                                     {
                                        "rewrite" : "rewrite",
